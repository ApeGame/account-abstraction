...
contract TokenPaymaster is BasePaymaster, UniswapHelper, OracleHelper {
    ...
    function setTokenPaymasterConfig(
        ...
        require(
            _tokenPaymasterConfig.priceMarkup <= 2 * PRICE_DENOMINATOR,
            "TPM: price markup too high"
        );
        require(
            _tokenPaymasterConfig.priceMarkup >= PRICE_DENOMINATOR,
        ...
        returns (bytes memory context, uint256 validationResult)
    {
        unchecked {
            ...
            // max cost + refundPostopCost * maxFeePerGas
            ...
            if (paymasterAndDataLength == 32) {
                // paymasterAndDataLength ==32 , 说明有客户端有 传入价格过来
            ...
            // 将 收取的 gas fee 转换为 对应 的 token 数量
            uint256 tokenAmount = weiToToken(
                preChargeNative,
                cachedPriceWithMarkup
            );
            // 将对应 的token 数量转给 paymaster
            ...
            // 上下文中 记录 了 手续的token 数量. gas price 和 抽象账号
            context = abi.encode(
                tokenAmount,
                userOp.maxFeePerGas,
            ...
            // uint48(0)<<208 | uint48(cachedPriceTimestamp + tokenPaymasterConfig.priceMaxAge) << 160
            // entry point 验证不可能成功的
    ...
    function getGasPrice(uint256 maxFeePerGas, uint256 maxPriorityFeePerGas)
        internal
        view
        returns (uint256)
    {
