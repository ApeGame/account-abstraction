...
function _parseValidationData(uint validationData)
    pure
    returns (ValidationData memory data)
{
    // aggregator = validationData[12:32]
    address aggregator = address(uint160(validationData));
    // uint48(uint96(bytes12(bytes32(validationData))))
    // validUntil = validationData[6:12]
    uint48 validUntil = uint48(validationData >> 160);
    if (validUntil == 0) {
        // 如果 validUntil == 0: type(uint48).max;
        validUntil = type(uint48).max;
    }
    // validAfter = validationData[0:6]
